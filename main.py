...
def main():
    ...
    try:
        target = input(f"{Fore.YELLOW}│ {Fore.WHITE}Enter target number (e.g., 1000): {Style.RESET_ALL}")
        target = int(target)
        if target <= 0:
            raise ValueError
    except:
        print(f"{Fore.YELLOW}│ {Fore.RED}✗ Invalid target number. Using unlimited mode.{Style.RESET_ALL}")
        target = None
    print(f"{Fore.YELLOW}└─────────────────────────────────────────────────────────────┘{Style.RESET_ALL}")
    ...
    while True:
        try:
            ...
            if target:
                ...
                if remaining > 0:
                    print(f"{Fore.YELLOW}│ {Fore.CYAN}📊 Progress: {order_count}/{target} ({remaining} remaining){Style.RESET_ALL}")
                else:
                    print(f"{Fore.YELLOW}│ {Fore.GREEN}🎯 Target reached! {order_count}/{target} orders completed{Style.RESET_ALL}")
                    break
            else:
                print(f"{Fore.YELLOW}│ {Fore.CYAN}📊 Progress: {order_count} orders completed{Style.RESET_ALL}")
